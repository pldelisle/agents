...
const skillCatalog = JSON.parse(
  readFileSync(resolve(moduleDirectory, "../skills/catalog.json"), "utf8"),
);
const SKILL_ALIASES = new Map(
  skillCatalog.skills.flatMap(({ id, aliases = [] }) =>
...
function requestedSkills(availableSkills, requested) {
  ...
  const canonicalRequested = unique(
    requested.map((skill) => SKILL_ALIASES.get(skill) ?? skill),
  );
  const missingSkills = canonicalRequested.filter(
