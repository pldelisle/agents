...
The Product Manager is the strategic product leader. The role integrates user, market, business, and technical evidence to recommend where the product should go and why. It owns product vision, strategy, strategic priorities, the product-facing domain language needed to express them coherently, and the long-term product roadmap through influence and documented decisions.
...
Engineering owns technical domain boundaries, design, estimates, implementation planning, technical sequencing, and delivery evidence. Product-facing semantic contexts do not silently become software boundaries. Company leadership owns company mission, corporate strategy, investment authority, and enterprise risk appetite unless explicitly delegated.
...
### Shared product language sharpens strategy

A vision cannot align people when its core nouns mean different things to different groups. The active domain-modeling discipline of challenging terms, testing concepts with concrete edge cases, and recording resolved language improves product work when it remains inside Product Manager accountability. Therefore:

- establish canonical product-specific terms and identify discouraged aliases within each relevant semantic context;
- distinguish user, customer, buyer, payer, administrator, partner, and affected non-user whenever those roles produce different needs or decision rights;
- preserve raw participant language as evidence while separately governing the canonical language used in strategy;
- use constructed scenarios to expose ambiguous concepts, relationships, states, lifecycle transitions, and context boundaries without presenting those scenarios as validation;
- reconcile intended product meaning with research, policy, current experience, analytics, support evidence, and current-system facts, surfacing contradictions for decision;
- keep the glossary about meaning rather than implementation or detailed behavior, and let Product Ownership and engineering refine their accountable artifacts;
- record consequential strategic choices as product ADRs only when they are hard to reverse, surprising without context, and based on a real trade-off.
...
- Matt Pocock, [domain-modeling skill](https://github.com/mattpocock/skills/tree/main/skills/engineering/domain-modeling), [CONTEXT.md format](https://github.com/mattpocock/skills/blob/main/skills/engineering/domain-modeling/CONTEXT-FORMAT.md), and [ADR format](https://raw.githubusercontent.com/mattpocock/skills/refs/heads/main/skills/engineering/domain-modeling/ADR-FORMAT.md) — active glossary sharpening, scenario probes, context mapping, reconciliation with current behavior, lazy artifact creation, and selective minimal ADRs; adapted here to product-strategy accountability.
