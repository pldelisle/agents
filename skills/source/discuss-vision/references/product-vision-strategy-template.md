...
3. Vision mandate, product language, and opportunity landscape
...
7. Handoff, product ADRs, and quality gate
...
### Product domain language and contexts

Use the `CONTEXT.md` format and repository layout defined by the domain-language protocol. A single-context repository uses root `CONTEXT.md`; a multi-context repository uses root `CONTEXT-MAP.md` linking each context glossary. Update the applicable glossary as terms resolve during discussion and record its exact revision here; do not duplicate definitions in this brief. Each glossary contains only its context name and short scope, natural cluster headings, canonical terms, concise definitions, and avoided aliases—never evidence, rationale, rules, scenarios, implementation types, schemas, services, events, or detailed behavior.

- **Canonical glossary revision:** <applicable CONTEXT.md path and revision>
- **Terms changed in this revision:** <canonical terms only; link rather than copying definitions>
- **Unresolved semantic conflicts:** <conflict, owner, and next decision gate; keep details outside CONTEXT.md>

#### Product context map

- **Root map revision:** <CONTEXT-MAP.md path/revision, or none>
- **Affected contexts:** <linked context glossary paths/revisions>
- **Unresolved translations:** <semantic relationship, owner, and next decision gate>

Create the root map only when one vocabulary cannot express the product coherently. Keep its relationships semantic; product contexts need not match teams, services, repositories, events, shared types, or data ownership.

#### Constructed scenario probes

| Probe ID | Context and starting state | Event or edge case | Concept/boundary stressed | Resolution or open question | Evidence status |
|---|---|---|---|---|---|

Set evidence status to `Constructed probe — not research evidence`. Move detailed acceptance examples to Product Ownership.
...
- **Product context and canonical terms:** <context, actor, concept, and relevant glossary revision>
...
- **Product glossary/context revision:** <path/revision, canonical terms, avoided aliases, unresolved conflicts>
- **Scenario probes and product ADRs:** <probe IDs and docs/adr links relevant to refinement>
...
Keep routine, reversible choices in this log. The expected default is no product ADR. Create or link one only when the choice is hard to reverse, surprising without its rationale, and the result of a genuine trade-off. Store qualifying records as `docs/adr/NNNN-slug.md` using a title and a one- to three-sentence context, decision, and rationale. Keep owner, date, evidence, and revisit information in this log.

| Decision ID | Date | Decision and owner | Alternatives | Evidence | Consequences | Revisit trigger | Product ADR/status |
|---|---|---|---|---|---|---|---|
...
- [ ] Canonical product terms, avoided aliases, semantic contexts, and translations are precise enough to prevent strategic ambiguity.
- [ ] Resolved vocabulary was written to the applicable `CONTEXT.md` before dependent reasoning; the required single- or multi-context root layout is in place.
- [ ] Each glossary follows the required context-heading, short-description, `## Language`, `**Term**:`, and optional `_Avoid_` format and contains vocabulary only.
- [ ] Consequential relationships, states, exceptions, and boundaries were challenged with constructed scenarios that are clearly separated from research evidence.
- [ ] Contradictions among the glossary, research, policy, current product behavior, analytics, support evidence, and current-system facts are resolved or assigned.
...
- [ ] Every product ADR meets the hard-to-reverse, surprise, and real-trade-off threshold; routine choices remain in the decision log.
- [ ] Product ADRs use sequential `docs/adr/NNNN-slug.md` names and the minimal title-plus-paragraph format; optional sections appear only when useful.
- [ ] No ADR was created merely for ceremony; zero ADRs is acceptable.
