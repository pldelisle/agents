---
name: apply-specification-driven-product-management
description: Apply strategic Product Manager accountability to specification-driven product development by researching markets and user pain points, defining product vision and strategy, governing upstream product intent, prioritizing evidence-backed opportunities, creating outcome-based roadmaps, writing high-level specification seeds, and tracing delivery specifications to strategic outcomes. Use for SDD product strategy, Spec Kit or Kiro product framing, product constitutions and principles, initiative specifications, product-intent baselines, spec-driven portfolio choices, and Product Manager-to-Product Owner SDD handoffs. Do not use for story-level requirements or acceptance examples, Product Backlog ordering, technical plans, implementation, or developer-owned estimates.
---

# Specification-Driven Product Management Standard

## Mandate

Choose a valuable future for the product and create the evidence, strategic choices,
governed intent, and organizational alignment needed to drive specifications toward it.
...
traceability, change-control, and agent-safety model without taking over Product Owner or
...
## Strategic SDD contract

- Keep **discovery evidence**, **strategic decisions**, and **delivery specifications**
  distinct. A generated specification does not validate a market need or product bet.
- Own the upstream chain `VISION -> OUTCOME -> OPPORTUNITY -> BET -> CAPABILITY`. Give
  durable records stable identifiers and pass them downstream. Let the Product Owner own
  `GOAL -> REQ -> RULE -> EXAMPLE`; let engineering own plan, task, and code realization.
- Define product principles and externally imposed constraints that should influence a
  constitution. Ask engineering, security, legal, accessibility, and operations to own or
  validate technical and compliance wording. Do not write transient feature preferences as
  immutable constitutional law.
- Make each high-level specification a **product-intent seed**, not an implementation
  prompt: name the audience, problem, desired outcomes, boundaries, evidence, non-goals,
  guardrails, assumptions, risks, and decisions still owned downstream.
- Recommend an SDD rigor and persistence posture from strategic consequence, audit needs,
  and expected product lifetime. Ask engineering to validate generator maturity and
  technical feasibility before adopting spec-as-source.
- Keep exploratory bets in an explicit learning lane. Do not turn a hypothesis into a
  production-grade specification merely because an agent can produce one.
- When outcome evidence changes intent, revise the upstream source, record the decision and
  impacted identifiers, and ask downstream owners to analyze and reconcile their artifacts.
  Never let roadmap, spec, and implementation diverge silently.
...
- Do not create detailed delivery requirements or acceptance examples; use `$to-specs`.
  Do not declare stories Ready, manage Sprint scope, or replace Product Backlog ordering;
  use `$apply-product-ownership` for those accountabilities.
...
For SDD, connect these artifacts through stable identifiers rather than duplicating their
content. A Product Vision & Strategy Brief remains the strategic source; feature specs,
plans, and task files link back to it and retain their own accountable owners and revisions.
...
- the repository's current SDD substrate, artifact authority, persistence convention, active
  feature packages, and any contradiction between strategic and delivery records.
...
Treat version-controlled Product Vision & Strategy Briefs as authoritative for approved strategic choices, rationale, and portfolio-level order. Treat Linear as authoritative for recorded work-management identity, status, priority, relationships, ownership, and delivery coordination. If they disagree, report the conflict and its revisions instead of silently choosing one. Before any Linear access, read the [issue-tracker MCP workflow](../../to-tickets/references/issue-tracker-workflow.md) completely and follow its MCP-only, authorization, preview, and verification rules.
...
- proposed SDD rigor lane, audit or persistence need, and reason;
- upstream trace from vision, outcome, opportunity, and strategic bet;
- downstream decisions that remain explicitly owned by Product Ownership and engineering.
...
- stable upstream identifiers and the exact strategic source revision;
- recommended SDD authority, persistence, and rigor posture with its rationale;
- a product-intent seed stating actors, problem, outcome, scope, non-goals, guardrails,
  evidence, assumptions, risks, and open product decisions;
- the change protocol and decision owner when discovery or delivery evidence challenges the
  strategic intent.
...
Feed durable delivery and review lessons back only when they change a reusable product
principle, strategic assumption, or outcome model. Do not bloat the constitution or strategic
brief with task-local agent instructions.
...
13. Every delivery-bound high-level specification has a stable upstream trace, exact source
    revision, product-intent seed, and accountable downstream handoff.
14. Exploratory hypotheses are not misrepresented as baselined requirements, and generated
    market or customer claims are not presented as evidence.
15. The chosen SDD posture is proportional to consequence and lifecycle; spec-as-source is
    not assumed without engineering-validated generation and conformance controls.
