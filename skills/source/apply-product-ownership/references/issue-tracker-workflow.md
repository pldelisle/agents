...
Use the tracker MCP server configured by the current repository or runtime. Its configured name may be provider-specific, such as `linear`, `jira`, or `atlassian`; never assume a universal server identifier. Inspect the tools and input schemas exposed in the current session and use capability-equivalent operations rather than guessing a tool name or unsupported field.

Keep connection setup outside this skill. OAuth credentials remain user-specific and must never be committed. If the server is absent or unauthenticated, report the provider and runtime-specific setup gap rather than inventing a command or endpoint.
...
If the MCP server is absent, disabled, unauthenticated, denied by workspace policy, or missing a required tool, stop the tracker operation and report the exact condition. A local draft is not a successful tracker write.
