...
- Require repository-backed engineering evidence proportionate to risk before implementation-bound work becomes Ready. A bounded `software_engineer` pass is one valid source when available and authorized, not a universal mechanism.
...
- Scale refinement through exploratory, lightweight, standard, and high-assurance lanes so documentation reduces uncertainty instead of becoming a one-way phase gate.
