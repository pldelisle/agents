...
Use this workflow whenever Product Owner work reads or changes Jira.

## Source-of-truth boundaries

- Treat Jira as authoritative for work-item identity, title, workflow status, product priority, parentage, relationships, ownership, project or initiative placement, milestones, cycles, and delivery coordination.
- Treat a version-controlled story specification as authoritative for its detailed behavioral revision when one exists. Store its repository path and exact revision in the Jira issue.
- Keep the Jira description sufficient to understand the outcome, scope, readiness, acceptance references, and specification link without copying a long specification that will drift.
- Preserve engineering ownership of estimates, technical tasks, implementation assignments, and capacity commitments.

## MCP-only access

Use the configured `Jira` MCP server at `https://mcp.atlassian.com/v1/mcp`. Inspect the tools and input schemas exposed in the current session; use capability-equivalent tools rather than assuming a tool name or unsupported field.

The repository commits only the server URL and approval policy. OAuth credentials remain user-specific and must never be committed. Register and authenticate once with `claude mcp add --transport http Jira https://mcp.atlassian.com/v1/mcp`; if the server is already registered for the user, run `claude mcp login Jira` instead. Complete the Jira OAuth flow and restart Claude so the server and tools load.

Never use browser automation, Jira's GraphQL or REST API, an unofficial CLI, or credentials stored in the repository as a fallback. Never expose OAuth tokens, API keys, private comments, customer data, or sensitive issue content in logs or local artifacts.

If the MCP server is absent, disabled, unauthenticated, denied by workspace policy, or missing a required tool, stop the Jira operation and report the exact condition. A local draft is not a successful Jira write.
...
| Product concept | Preferred Jira representation |
|---|---|
...
| Detailed behavioral specification | Version-controlled file linked with exact revision, or existing Jira document convention |
...
Do not assume the skill's lifecycle names match Jira workflow-state names. Discover the team's actual states and record the intended mapping before the first transition. Do not create replacement states merely to obtain a one-to-one mapping.
...
- re-read the issue and specification link, then record the verified Jira synchronization state in the specification.

If Jira and the repository disagree, do not silently choose one. Report the conflicting fields and timestamps or revisions, identify the accountable owner, and block affected mutations until the intended source is confirmed.

## Parallel delivery

When synchronizing an authorized parallel plan:

- create or update one independently assignable Jira issue or sub-issue per work package;
...
- never infer implementation-agent assignment from a Jira assignee unless the integration explicitly establishes that meaning.
