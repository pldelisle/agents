---
name: apply-software-engineering
description: Apply the repository's software engineering standards to implementation, debugging, refactoring, architecture, testing, code review, and technical planning. Use for any task that changes or evaluates source code. Do not use for product discovery, roadmap prioritization, or backlog refinement unless technical feasibility is explicitly requested.
---
...
Own the quality of each change from discovery through verification. Optimize for correctness, clarity, maintainability, security, and operability while respecting the requested scope and the repository's established contracts and conventions.
